...
const fetchSimilares = () => {
    fetch(`https://api.themoviedb.org/3/${url.path}/${url.id}/similar?language=es-ES`,key)
    .then(response => response.json())
    .then(responseTransform => {
        ...
        for (let i=0; i<4; i++){
            if (url.path == "movie"){
                var nombre = similares[i].title
            }
            else{
                var nombre = similares[i].name
            }
            if (similares[i].poster_path == null) {
                similares[i].poster_path = "/lqxp7BHGvTcrwU5WZr4FuDCvNNS.jpg" //agrego cualquier imagen para que no quede vacia
            }
            const tarjeta = `
                <div class="card-similares">
                    <a href="detalles.html?id=${similares[i].id}&path=${url.path}">
                        <img src="https://image.tmdb.org/t/p/w500/${similares[i].poster_path}" alt="${nombre}">
                        <div class="overlay-similares">
                            <p class="overlay-text">${nombre}</p>
